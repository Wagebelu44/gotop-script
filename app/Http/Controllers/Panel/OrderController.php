...
class OrderController extends Controller
{
    ...
    {
        if (
            Auth::user()->can('edit order link') 
            || Auth::user()->can('set start count order') 
            || Auth::user()->can('set remains order')
            || Auth::user()->can('change order status')
            || Auth::user()->can('set partial order')
            || Auth::user()->can('cancel and refund order')
            ) {
            try {
                $data = $request->all();
