...
class CreateSettingGeneralsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('setting_generals', function (Blueprint $table) {
            ...
            $table->string('panel_name')->nullable()->default('Go Panel Shop');
            $table->string('timezone')->nullable()->default(0);
            $table->string('currency')->nullable()->default('USD');
            $table->string('currency_sign')->nullable()->default('$');
            $table->string('currency_name')->nullable()->default('US Dollar');
            $table->string('currency_format')->nullable();
            $table->string('rates_rounding')->nullable();
            $table->unsignedTinyInteger('ticket_system')->nullable()->default(1)->comment('0 => Disabled, 1 => Enabled');
            $table->string('tickets_per_user')->nullable();
            $table->unsignedTinyInteger('signup_page')->nullable()->default(1)->comment('0 => Disabled, 1 => Enabled');
            $table->unsignedTinyInteger('email_confirmation')->nullable()->default(1)->comment('0 => Disabled, 1 => Enabled');
            $table->unsignedTinyInteger('skype_field')->nullable()->comment('0 => Disabled, 1 => Enabled');
            $table->unsignedTinyInteger('name_fields')->nullable()->default(1)->comment('0 => Disabled, 1 => Enabled');
            $table->unsignedTinyInteger('terms_checkbox')->nullable()->default(1)->comment('0 => Disabled, 1 => Enabled');
            $table->unsignedTinyInteger('reset_password')->nullable()->default(1)->comment('0 => Disabled, 1 => Enabled');
